...
class IosAndroidUiParityTest {
    ...
    fun receiveHistoryDeletionUsesSwipeConfirmationAndSavedFileReferencesOnBothPlatforms() {
        ...
        assertTrue("pendingDeleteItem = item" in iosReceive)
        assertTrue(".tint(.red)" in iosReceive)
        assertTrue(".alert(pendingDeleteItem?.deleteConfirmationTitle" in iosReceive)
        assertTrue("nativeReceiveSwipeListRow" in iosReceive)
        assertTrue("EdgeInsets(top: top, leading: 24, bottom: bottom, trailing: 8)" in iosReceive)
        assertTrue("confirmDeleteReceiveHistory(item, deleteFiles: false)" in iosReceive)
        assertTrue("confirmDeleteReceiveHistory(item, deleteFiles: true)" in iosReceive)
        ...
        assertTrue("Button(\"仅删除记录\"" in iosReceive)
        assertTrue("Button(\"删除记录与文件\"" in iosReceive)
