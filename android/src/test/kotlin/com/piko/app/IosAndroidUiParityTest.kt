...
class IosAndroidUiParityTest {
    ...
    fun receiveHistoryDeletionUsesSwipeConfirmationAndSavedFileReferencesOnBothPlatforms() {
        ...
        assertTrue("UIViewController, UITableViewDataSource, UITableViewDelegate" in iosReceive)
        assertTrue("private let tableView = UITableView(frame: .zero, style: .plain)" in iosReceive)
        assertTrue("tableView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -NativeReceiveLayout.pageHorizontalInset)" in iosReceive)
        ...
        assertTrue("static let historyRowHeight: CGFloat = 84" in iosReceive)
        assertTrue("nextRows.append(.gap(NativeReceiveLayout.historyRowSpacing))" in iosReceive)
        assertTrue("tableView.register(UITableViewCell.self, forCellReuseIdentifier: \"gap\")" in iosReceive)
        ...
        assertTrue("case .history:\n            return NativeReceiveLayout.historyRowHeight" in normalizedIosReceive)
        ...
        assertFalse("historySwipeCardGap" in iosReceive)
        assertFalse("willBeginEditingRowAt" in iosReceive)
        assertFalse("didEndEditingRowAt" in iosReceive)
        ...
        assertTrue("case .hero:\n            return \"hero\"" in normalizedIosReceive)
        assertTrue("case .deviceName:\n            return \"deviceName\"" in normalizedIosReceive)
        assertTrue("case .empty:\n            return \"empty\"" in normalizedIosReceive)
        assertTrue("case .active:\n            return \"active\"" in normalizedIosReceive)
        assertFalse("return \"content\"" in iosReceive)
        assertTrue("tableView.register(NativeHostingTableCell.self, forCellReuseIdentifier: \"hero\")" in iosReceive)
        assertTrue("tableView.register(NativeHostingTableCell.self, forCellReuseIdentifier: \"deviceName\")" in iosReceive)
        assertTrue("tableView.register(NativeHostingTableCell.self, forCellReuseIdentifier: \"empty\")" in iosReceive)
        assertTrue("tableView.register(NativeHostingTableCell.self, forCellReuseIdentifier: \"active\")" in iosReceive)
        assertFalse("forCellReuseIdentifier: \"content\"" in iosReceive)
        assertTrue("private var pendingRows: [NativeReceiveTableRow]?" in iosReceive)
        assertTrue("private var needsDeferredReload = false" in iosReceive)
        assertTrue("private var pendingAnimatedDeletedHistoryID: UUID?" in iosReceive)
        assertTrue("DispatchQueue.main.async { [weak self] in" in iosReceive)
        assertTrue("UIView.performWithoutAnimation" in iosReceive)
        assertTrue("private func applyRows(_ nextRows: [NativeReceiveTableRow])" in iosReceive)
        assertTrue("animatePendingDeletedHistoryIfNeeded(nextRows)" in iosReceive)
        assertTrue("private func animatePendingDeletedHistoryIfNeeded(_ nextRows: [NativeReceiveTableRow]) -> Bool" in iosReceive)
        assertTrue("pendingAnimatedDeletedHistoryID = item.id" in iosReceive)
        assertTrue("pendingAnimatedDeletedHistoryID = nil" in iosReceive)
        assertTrue("tableView.performBatchUpdates" in iosReceive)
        assertTrue("tableView.deleteRows(at: deletedIndexPaths, with: .fade)" in iosReceive)
        assertTrue("tableView.insertRows(at: insertedIndexPaths, with: .fade)" in iosReceive)
        ...
        assertTrue("        pendingAnimatedDeletedHistoryID = item.id\n        onDeleteReceiveHistory" in normalizedIosReceive)
        assertFalse("swipeCompletion(true)" in iosReceive)
        assertFalse("tableView.deleteRows(at: deletedIndexPaths, with: .automatic)" in iosReceive)
    ...
    fun iosReceiveRootUsesPageBackgroundThroughSystemBars() {
        ...
        assertTrue("static func emptyStateRowHeight(for tableHeight: CGFloat) -> CGFloat" in iosReceive)
        assertTrue("static let emptyStateHeightCacheKeyPrefix = \"piko.receive.emptyStateHeight\"" in iosReceive)
        assertTrue("private var cachedEmptyStateRowHeight: CGFloat?" in iosReceive)
        assertTrue("cachedEmptyStateRowHeight = NativeReceiveLayout.cachedEmptyStateRowHeight(for: tableView)" in iosReceive)
        assertTrue("NativeReceiveLayout.persistEmptyStateRowHeight(nextHeight, for: tableView)" in iosReceive)
        assertTrue("UserDefaults.standard.double(forKey: key)" in iosReceive)
        assertTrue("UserDefaults.standard.set(Double(height), forKey: key)" in iosReceive)
        assertTrue("tableView.window?.screen ?? UIScreen.main" in iosReceive)
        assertTrue("NativeReceiveLayout.emptyStateRowHeight(for: tableView.bounds.height)" in iosReceive)
        assertTrue("emptyStateCardView(" in iosReceive)
        assertTrue("private struct NativeReceiveEmptyStateContent: View" in iosReceive)
        assertTrue("return AnyView(\n            content()" in iosReceive)
        ...
        assertTrue(".frame(height: height, alignment: .top)" in iosReceive)
