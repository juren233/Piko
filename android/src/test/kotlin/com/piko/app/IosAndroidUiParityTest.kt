...
class IosAndroidUiParityTest {
    ...
    @Test
    fun iosReceiveFileListUsesPureSwiftUIStructureAndKeepsRequiredControls() {
        val androidReceive = File(rootDir, "android/src/main/kotlin/com/piko/app/ReceiveScreen.kt").readText()
        val iosReceive = readIos("NativeReceiveView.swift")

        assertTrue("modifier = Modifier.weight(1f)" in androidReceive)
        assertTrue("ScrollView" in iosReceive)
        assertTrue("LazyVStack" in iosReceive)
        assertTrue("ForEach(Array(model.receiveHistory.enumerated()), id: \\.element.id)" in iosReceive)
        assertTrue("NativeReceiveSwipeRow" in iosReceive)
        assertTrue("DragGesture(minimumDistance: 12)" in iosReceive)
        assertTrue("NativeReceiveHistoryCard(item: item)" in iosReceive)
    ...
    fun receiveHistoryDeletionUsesSwipeConfirmationAndSavedFileReferencesOnBothPlatforms() {
        ...
        assertTrue("ForEach(Array(model.receiveHistory.enumerated()), id: \\.element.id)" in iosReceive)
        assertTrue("bottomSpacing: index < model.receiveHistory.count - 1 ? NativeReceiveLayout.historyRowSpacing : 0" in iosReceive)
    ...
    fun iosReceiveRootUsesPageBackgroundThroughSystemBars() {
        ...
        assertTrue("ScrollView(showsIndicators: false)" in iosReceive)
