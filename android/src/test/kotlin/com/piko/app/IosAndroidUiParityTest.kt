...
class IosAndroidUiParityTest {
    ...
    fun receivePagesUseCompactActiveProgressAndMediaPreview() {
        ...
        assertTrue("mediaPreviewImageData(" in iosModel)
        assertTrue("UIGraphicsImageRenderer" in iosModel)
        assertTrue("jpegData(compressionQuality: 0.82)" in iosModel)
        assertFalse("case .image:\n            return fallbackData" in iosModel.replace("\r\n", "\n"))
    ...
    fun iosReceiveFileListUsesNativeTableAndKeepsRequiredControls() {
        ...
        assertTrue("NativeReceiveTable(model: model" in iosReceive)
        ...
        assertTrue("private final class NativeReceiveSpacerCell: UITableViewCell" in iosReceive)
        assertTrue("return NativeReceiveSpacerCell(" in iosReceive)
