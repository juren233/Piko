...
std::unordered_map<int64_t, std::shared_ptr<NativeContext>> gContexts;
std::unordered_map<int64_t, std::shared_ptr<NativeChannel>> gChannels;
...
std::shared_ptr<NativeChannel> findChannel(NativeChannel *raw) {
    ...
    auto it = gChannels.find(reinterpret_cast<int64_t>(raw));
    return it == gChannels.end() ? nullptr : it->second;
}

void registerChannel(const std::shared_ptr<NativeChannel> &channel) {
    std::lock_guard<std::mutex> lock(gRegistryMutex);
    gChannels[reinterpret_cast<int64_t>(channel.get())] = channel;
}
...
void unregisterChannel(const std::shared_ptr<NativeChannel> &channel) {
    if (!channel) return;
    std::lock_guard<std::mutex> lock(gRegistryMutex);
    gChannels.erase(reinterpret_cast<int64_t>(channel.get()));
}

void notifyClosed(const std::shared_ptr<NativeChannel> &channel) {
    if (!channel || channel->context == nullptr || channel->context->onClosed == nullptr) return;
    channel->context->onClosed(reinterpret_cast<int64_t>(channel.get()), channel->context->userData);
}

void notifyFrame(const std::shared_ptr<NativeChannel> &channel, const uint8_t *data, size_t size) {
    if (!channel || channel->context == nullptr || channel->context->onFrame == nullptr || size > kMaxFrameBytes) return;
    channel->context->onFrame(
        reinterpret_cast<int64_t>(channel.get()),
        data,
        static_cast<int32_t>(size),
        channel->context->userData
    );
}
...
void readFrames(const std::shared_ptr<NativeChannel> &channel) {
    ...
    while (channel->stream != nullptr) {
        ...
        if (read <= 0) {
            if (read < 0) channel->closed.store(true);
            break;
        }
...
void drainChannelSends(const std::shared_ptr<NativeChannel> &channel) {
    ...
    while (!channel->pending.empty() && channel->stream != nullptr && !channel->closed.load()) {
...
void closeContext(const std::shared_ptr<NativeContext> &ctx) {
    ...
    std::lock_guard<std::mutex> lock(gRegistryMutex);
    gContexts.erase(reinterpret_cast<int64_t>(ctx.get()));
    for (auto &channel : ctx->channels) gChannels.erase(reinterpret_cast<int64_t>(channel.get()));
}

} // namespace

extern "C" int32_t piko_xquic_is_linked(void) {
    return 1;
}

extern "C" int64_t piko_xquic_open_server(
    ...
    {
        std::lock_guard<std::mutex> lock(gRegistryMutex);
        gContexts[reinterpret_cast<int64_t>(ctx.get())] = ctx;
    }
    ctx->worker = std::thread(loopContext, ctx);
    return reinterpret_cast<int64_t>(ctx.get());
}

extern "C" int32_t piko_xquic_server_port(int64_t server_handle) {
    std::lock_guard<std::mutex> lock(gRegistryMutex);
...
extern "C" int64_t piko_xquic_open_client(
    const char *host,
    int32_t port,
    ...
    ctx->worker = std::thread(loopContext, ctx);
    const int waitMillis = timeout_millis > 0 ? timeout_millis : 5000;
    {
        std::unique_lock<std::mutex> lock(ctx->mutex);
        const bool ready = ctx->cv.wait_for(
            ...
            [&] {
                return channel->handshakeComplete.load() || channel->closed.load() || ctx->closed.load();
            }
        );
        if (!ready || channel->closed.load() || ctx->closed.load()) {
            closeContext(ctx);
            return 0;
        }
    }
    return reinterpret_cast<int64_t>(channel.get());
}

extern "C" int32_t piko_xquic_send_frame(int64_t channel_handle, const uint8_t *data, int32_t size) {
    std::shared_ptr<NativeChannel> channel;
    {
        std::lock_guard<std::mutex> lock(gRegistryMutex);
        auto it = gChannels.find(channel_handle);
        if (it != gChannels.end()) channel = it->second;
    }
    if (!channel || data == nullptr || size <= 0 || static_cast<size_t>(size) > kMaxFrameBytes) return 0;
    ...
    std::memcpy(pending.bytes.data() + 4, data, static_cast<size_t>(size));
    {
        std::lock_guard<std::mutex> lock(channel->mutex);
        if (channel->closed.load()) return false;
...
extern "C" void piko_xquic_close_channel(int64_t channel_handle) {
    std::shared_ptr<NativeChannel> channel;
    {
        std::lock_guard<std::mutex> lock(gRegistryMutex);
        auto it = gChannels.find(channel_handle);
    ...
    if (channel->clientOwnedContext && channel->context != nullptr) {
        ...
        {
            std::lock_guard<std::mutex> lock(gRegistryMutex);
            auto it = gContexts.find(reinterpret_cast<int64_t>(channel->context));
            if (it != gContexts.end()) ctx = it->second;
        }
        closeContext(ctx);
        return;
